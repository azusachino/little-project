...
func FindOrCreateFile(fileMd5 string, fileName string, chunkTotal int) (err error, file model.ExaFile) {
	...
	notHaveSameMd5Finish := global.DB.Where("file_md5 = ? AND is_finish = ?", fileMd5, true).First(&file).RecordNotFound()
...
func FileCreateComplete(fileMd5 string, fileName string, filePath string) error {
	...
	err := global.DB.Where("file_md5 = ? AND file_name = ?", fileMd5, fileName).First(&file).Updates(upDateFile).Error
	return err
}
