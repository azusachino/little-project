...
func CreateApi(api model.SysApi) (err error) {
	findOne := global.DB.Where("path = ? AND method = ?", api.Path, api.Method).Find(&model.SysApi{}).Error
	if findOne == nil {
...
func UpdateApi(api model.SysApi) (err error) {
	...
	if oldA.Path != api.Path || oldA.Method != api.Method {
		flag := global.DB.Where("path = ? AND method = ?", api.Path, api.Method).Find(&model.SysApi{}).RecordNotFound()
		if !flag {
