...
import (
	...
	"little-gin-admin/model/response"
	"strconv"
)
...
func CreateAuthority(auth model.SysAuthority) (err error, authority model.SysAuthority) {
	...
	notHas := global.DB.Where("authority_id = ?", auth.AuthorityId).Find(&authorityBox).RecordNotFound()
	if !notHas {
...
func CopyAuthority(copyInfo response.SysAuthorityCopyResponse) (err error, authority model.SysAuthority) {
	var authorityBox model.SysAuthority
	notHas := global.DB.Where("authority_id = ?", copyInfo.Authority.AuthorityId).Find(&authorityBox).RecordNotFound()
	if !notHas {
...
func DeleteAuthority(auth *model.SysAuthority) (err error) {
	err = global.DB.Where("authority_id = ?", auth.AuthorityId).Find(&model.SysUser{}).Error
	if err == nil {
		err = errors.New("此角色有用户正在使用禁止删除")
		return
	}
	err = global.DB.Where("parent_id = ?", auth.AuthorityId).Find(&model.SysAuthority{}).Error
	if err == nil {
		err = errors.New("此角色存在子角色不允许删除")
		return
	}
	db := global.DB.Preload("SysBaseMenus").Where("authority_id = ?", auth.AuthorityId).First(auth).Unscoped().Delete(auth)
...
func SetDataAuthority(auth model.SysAuthority) error {
	...
	err := global.DB.Model(&s).Association("DataAuthorityId").Replace(&auth.DataAuthorityId).Error
	return err
}
...
func SetMenuAuthority(auth *model.SysAuthority) error {
	...
	err := global.DB.Model(&s).Association("SysBaseMenus").Replace(&auth.SysBaseMenus).Error
	return err
}
