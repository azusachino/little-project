...
func Register(u model.SysUser) (err error, userInter model.SysUser) {
	...
	// 判断用户名是否注册
	notRegister := global.DB.Where("username = ?", u.Username).First(&user).RecordNotFound()
...
func UploadHeaderImg(uuid uuid.UUID, filePath string) (err error, userInter *model.SysUser) {
	var user model.SysUser
	err = global.DB.Where("uuid = ?", uuid).First(&user).Update("header_img", filePath).First(&user).Error
	return err, &user
}
