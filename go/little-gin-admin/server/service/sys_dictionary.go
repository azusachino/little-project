...
func CreateSysDictionary(sysDictionary model.SysDictionary) (err error) {
	if (!global.DB.First(&model.SysDictionary{}, "type = ?", sysDictionary.Type).RecordNotFound()) {
...
func DeleteSysDictionary(sysDictionary model.SysDictionary) (err error) {
	err = global.DB.Delete(sysDictionary).Related(&sysDictionary.SysDictionaryDetails).Delete(&sysDictionary.SysDictionaryDetails).Error
	return err
}
